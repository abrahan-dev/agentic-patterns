...
import { DeterministicWorkspaceBootstrapper } from "../../src/local/workspace-bootstrapper.ts";
...
describe("deterministic workspace bootstrapper", () => {
  test("creates the fixed full-stack topology in a metadata-only workspace", async () => {
    ...
    expect(result.createdFiles).toContain(
      "test/apps/purchase-orders/backend/server.test.ts",
    );
  ...
  test("can resume validation after a local command interruption", async () => {
    ...
    expect(interruptedBootstrapper.bootstrap(root, plan)).rejects.toThrow(
      "bun run typecheck",
    );
    shouldFail = false;
