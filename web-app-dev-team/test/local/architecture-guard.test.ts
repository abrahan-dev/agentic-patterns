...
import { checkArchitecture } from "../../src/local/architecture-guard.ts";
...
test("detects forbidden layer imports, domain frameworks, cycles and misplaced tests", async () => {
  ...
  await writeFile(
    resolve(root, "src", "orders", "domain", "order.test.ts"),
    "export {};\n",
  );
  ...
  expect(
    violations.some((value) => value.includes("domain cannot import infrastructure")),
  ).toBe(true);
  expect(
    violations.some((value) => value.includes("domain imports framework package")),
  ).toBe(true);
  expect(violations.some((value) => value.includes("Circular dependency"))).toBe(true);
  expect(violations.some((value) => value.includes("tests must live outside src"))).toBe(
    true,
  );
});
...
test("enforces context/app topology and prevents frontend persistence access", async () => {
  ...
  expect(violations.some((value) => value.includes("src/contexts or src/apps"))).toBe(
    true,
  );
  expect(violations.some((value) => value.includes("must select application"))).toBe(
    true,
  );
