...
import { CodexAgentRunner } from "../agents/codex-agent-runner.ts";
import { ScriptedAgentRunner } from "../demo/scripted-agent-runner.ts";
import type { TokenTotals } from "../domain/schemas.ts";
import { runDevelopmentTeam } from "../orchestration/orchestrator.ts";
import { createRunState, loadRunState } from "../orchestration/run-store.ts";
...
} from "../specifications/specification-journal.ts";
import { assertTmuxInstalled, BunCommandRunner, launchTmux } from "../ui/tmux.ts";
...
function tokenSummary(totals: TokenTotals): string {
  ...
  return [
    ...
    ...Object.entries(totals.byRole).map(
      ([role, usage]) => `${role} ${count(usage.totalTokens)}`,
    ),
  ].join(" · ");
}
...
async function launchRestitution(
  ...
  if (existingDirectory) {
    ...
    const created = await createRestitution({
      ...
      specificationsDirectory: specificationsDirectory(
        arguments_.required("--specs-path"),
      ),
...
async function startDevelopment(arguments_: CliArguments, demo: boolean): Promise<void> {
  const prompt =
    arguments_.optional("--prompt") ??
    (demo ? "Add a generic feature using TDD." : undefined);

  if (!prompt) {
    throw new Error(
      'Provide the task with --prompt "...". It may describe any software feature.',
    );
  }
