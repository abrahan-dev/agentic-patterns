...
function executionObservations(
  ...
  return {
    ...
    changedFiles: normalizeChangedFiles(
      observations ? observations.changedFiles : [],
      workspace,
    ),
  };
}
...
function unpackAgentResult(raw: Awaited<ReturnType<AgentRunner["run"]>>): {
  ...
  return isAgentRunResult(raw)
    ? { result: raw, turn: raw.turn }
    : { result: null, turn: raw };
}
...
export async function executeAgentTurn(options: {
  ...
  const { runner, runDirectory, state, journal, attempt } = options;
  ...
  await recordTurnStarted(runDirectory, state, role);
  ...
  await recordTurnCompleted(runDirectory, state, role, result?.usage ?? null);
...
export async function recordFailedAttempt(
  ...
  await saveRunState(runDirectory, state);
  await recordRunFailure(runDirectory, state, attempt.activeRole, failure);
}
