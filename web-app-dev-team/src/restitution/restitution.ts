...
async function installSpecificationArchive(
  ...
  if (await exists(target)) {
    ...
    if (entries.length > 0) {
      throw new Error(
        `Refusing to replace the non-empty directory ${target} without a manifest.`,
      );
    }
  }
...
export async function createRestitution(options: {
  ...
  const specifications = await installSpecificationArchive(
    sourceSpecifications,
    workspace,
  );
  const id = `${Date.now()}-${safeId(basename(sourceSpecifications)) || "specifications"}`;
  const directory = resolve(
    options.runsRoot ?? workspace,
    ".web-app-dev-team",
    "restitutions",
    id,
  );
  ...
  await Promise.all(
    roles.map((role) => Bun.write(resolve(directory, "logs", `${role}.log`), "")),
  );
...
async function reportProgress(directory: string, message: string): Promise<void> {
  ...
  await Promise.all(
    roles.map((role) =>
      appendFile(resolve(directory, "logs", `${role}.log`), `\n${line}\n`),
    ),
  );
}
...
async function finishRestitution(
  directory: string,
  restitution: RestitutionState,
): Promise<void> {
...
export async function runRestitution(
  ...
  const { restitution, recoveringActiveRun } = await beginRestitution(
    directory,
    maxTurnsOverride,
  );
