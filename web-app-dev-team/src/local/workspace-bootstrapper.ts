...
import { runLocalCommand } from "./quality-gate.ts";
import {
  enterpriseWebAppTemplate,
  enterpriseWebAppTemplateVersion,
} from "./templates/enterprise-web-app.ts";
...
function resultBase(plan: ChangePlan) {
  return {
    template: "enterprise-web-app" as const,
    templateVersion: enterpriseWebAppTemplateVersion,
...
async function readBootstrapMarker(path: string): Promise<BootstrapMarker | null> {
  ...
  if (
    marker.template !== "enterprise-web-app" ||
    marker.templateVersion !== enterpriseWebAppTemplateVersion
  ) {
    throw new Error(
...
async function runBootstrapValidation(
  ...
  for (const command of validationCommands) {
    ...
    if (result.exitCode !== 0) {
      throw new Error(
        `Workspace bootstrap command failed (${result.command}): ${result.output}`,
      );
    }
  }
...
function interruptedPlanMatches(
...
export class DeterministicWorkspaceBootstrapper implements WorkspaceBootstrapper {
  ...
  async bootstrap(workspace: string, plan: ChangePlan): Promise<WorkspaceBootstrap> {
    const template = enterpriseWebAppTemplate(plan);
    ...
    if (marker?.status === "complete") {
      return skippedBootstrap(
        plan,
        `Workspace was already initialized by template v${enterpriseWebAppTemplateVersion}.`,
      );
    }
    ...
    return {
      ...resultBase(plan),
      status: "created",
      reason: `Created, installed and locally validated template v${enterpriseWebAppTemplateVersion}.`,
      createdFiles,
...
export class NoopWorkspaceBootstrapper implements WorkspaceBootstrapper {
  bootstrap(workspace: string, plan: ChangePlan): Promise<WorkspaceBootstrap> {
    return Promise.resolve(
      skippedBootstrap(plan, `Bootstrap disabled for ${workspace}.`),
    );
  }
}
