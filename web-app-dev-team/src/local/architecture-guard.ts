...
async function scan(
  ...
  const matches = await Promise.all(
    patterns.map((pattern) =>
      Array.fromAsync(
        new Bun.Glob(pattern).scan({ cwd: workspace, absolute, onlyFiles: true }),
      ),
    ),
  );
...
async function analyzeSourceFile(
  ...
  for (const specifier of importedSpecifiers(source)) {
    ...
    violations.push(
      ...importRuleViolations(sourcePath, sourceLayer, targetLayer, specifier),
    );
  }
...
async function testMirrorViolations(
  ...
  for (const testFile of testFiles) {
    ...
    if (sourcePath && !sourceFiles.has(resolve(workspace, sourcePath))) {
      violations.push(
        `${testFile}: does not mirror an existing production path (${sourcePath}).`,
      );
    }
  }
