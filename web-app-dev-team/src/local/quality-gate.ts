...
import type { LocalCheck, LocalCommandResult, Role } from "../domain/schemas.ts";
import { checkArchitecture } from "./architecture-guard.ts";
import type { WorkspaceFacts } from "./workspace-inspector.ts";
...
export async function runLocalCommand(
  ...
  try {
    ...
    return {
      ...
      exitCode,
      output:
        output.length <= 4_000 ? output : `${output.slice(-4_000)}\n…output truncated`,
    };
  } catch (error) {
    return {
      command: command.join(" "),
...
async function complexityViolations(
  workspace: string,
  changedFiles: string[],
): Promise<string[]> {
  const candidates = [
  ...
  for (const script of selectedScripts) {
    commands.push(
      await runLocalCommand(
        commandFor(options.facts.packageManager, script),
        options.workspace,
      ),
    );
  }
